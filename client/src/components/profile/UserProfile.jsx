...
const fetchOrdersAndWishlist = async () => {
  ...
  try {
    const [ordersRes, wishlistRes] = await Promise.all([
      ...
      axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/wishlist`, {
        headers: { Authorization: `Bearer ${token}` },
      }),
    ]);
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.put(`${import.meta.env.VITE_API_BASE_URL}/api/users/me`, profileData, {
        headers: { Authorization: `Bearer ${token}` },
      });
...
const handleCancelOrder = async (orderId) => {
  ...
  const handleAvatarChange = async (e) => {
    ...
    if (file) {
      ...
      try {
        const res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/users/upload-avatar`, formData, {
          headers: {
  ...
  const handleAddressSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedAddress) {
        res = await axios.put(
         `${import.meta.env.VITE_API_BASE_URL}/api/users/addresses/${selectedAddress._id}`,
          addressData,
          { headers: { Authorization: `Bearer ${token}` } }
        );
      } else {
        res = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/users/addresses`, addressData, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
      setUserData((prev) => ({
  ...
  const handleDeleteAddress = async (addressId) => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/api/users/addresses/${addressId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setUserData((prev) => ({
        ...prev,
  ...
  try {
    ...
    const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/cart`, cartItem, { // Updated endpoint
      headers: { Authorization: `Bearer ${token}` },
    });
