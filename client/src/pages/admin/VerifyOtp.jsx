...
const VerifyOtp = () => {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5002/api/auth/verify-otp', { email, otp });
  ...
  const handleResendOtp = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5002/api/auth/send-otp', { email, role });
      toast.success(response.data.message);
    } catch (err) {
